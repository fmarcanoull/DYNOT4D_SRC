...
if ~exist('yTrials','var')
        yTrials = [];
        dodTrials = [];
else
    % Primero esto
    for ix = 1:length(yTrials)
        YB = yTrials(ix).yblk;
        R(:,:,:,ix) = reshape(permute(YB,[1 4 2 3]),[size(YB,1) * size(YB,4), size(YB,2), size(YB,3)]);
    end
    dodTrials = DOTHUB_hmrConc2OD(R,SD3D,DPFs);
    % Luego esto. Por ahora lo dejamos sin multiplicar
    %for ix =1:length(yTrials)                   %#ok<NODEF>
    %    yTrials(ix).yblk= 1e6*yTrials(ix).yblk; %#ok<AGROW>
    %end
end
[prepro, preproFileName] = DOTHUB_writePREPRO(preproFileName,logData,dodRecon,tRecon,SD3D,hmr.s,dcAvg,dcAvgStd,tHRF,condNames,SD2D,dodTrials,yTrials);
